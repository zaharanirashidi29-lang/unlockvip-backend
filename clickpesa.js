...
async function getAccessToken() {
  ...
  tokenCache = { token: data.token, expiry: now + 55 * 60 * 1000 };
  return data.token;
}
...
function formatClickpesaError(error) {
  const data = error.response?.data;
  return {
    message: data?.message || error.message || "ClickPesa request failed",
